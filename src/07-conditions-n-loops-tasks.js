...
function isInsideCircle(circle, point) {
  return circle.radius > Math.sqrt((circle.center.x - point.x) ** 2
...
function isCreditCardNumber(ccn) {
  ...
  if (arr.length % 2 === 0) {
    const res = arr.map((item, index) => (index % 2 === 0 ? +item * 2 : +item))
...
function getDigitalRoot(num) {
  ...
  const rez = arr.reduce((sum, item) => sum + +item, 0);
  const arr2 = rez.toString().split('');
  return arr2.reduce((sum, item) => sum + +item, 0);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  while (status) {
    for (let i = 0; i < pathes.length; i += 1) {
      if (pathes[0][idx] !== pathes[i][idx]) status = false;
    }
    if (status && pathes[0][idx] === '/') end = idx + 1;
